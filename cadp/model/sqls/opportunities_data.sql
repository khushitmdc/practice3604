...
cte2 AS (
    SELECT *,
        CASE
            WHEN DATE(rfi_release_date) >= current_date THEN
                CASE
...
ranked AS (
    SELECT *,
        ROW_NUMBER() OVER (
            ORDER BY  rfi_release_date ASC , priority_month ASC, pwin DESC
...
final AS (
    SELECT
        c3.*,
        CASE
            WHEN c3.priority_month = 0 THEN 0
            ELSE r.future_rank
        END AS priority_rank
    FROM cte4 c3
    LEFT JOIN ranked r ON c3.row_id = r.row_id
)
